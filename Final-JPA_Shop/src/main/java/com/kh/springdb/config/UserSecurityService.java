...
/*
	...
	인터페이스로 다양한 종류의 메서드가 있음
		...
		getAuthorities() : 사용자가 가지고 있는 권한 목록을 반환
							권한은 GrantedAuthority에서 가지고온다
							권한은 정의된 권한에 따라 달라짐, 권한은 개발자가 설정해준다
		getPassword() : 사용자의 비밀번호를 반환, 데이터베이스에서 암호화 처리된 형태로 저장돼있음
		getUsername() : 사용자명을 반환
		isEnable() : 계정이 활성화 돼있는지 여부를 Boolean 값으로 나타낸다
