...
public class SecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests((authorizeHttpRequests) -> (authorizeHttpRequests).requestMatchers(new AntPathRequestMatcher("/**")).permitAll()) // "/**" : 모든 폴더, permitAll() : 전체허용
		
		// ** 로그인을 하고 난 후 로그인 세션을 유지하기 위한 설정 **
		.formLogin((formLogin) -> formLogin.loginPage("/user/login").defaultSuccessUrl("/"))
		
		// ** 로그아웃하고 난 후에 로그인 세션을 끝내기 위한 설정 ** 
	...
	@Bean
	AuthenticationManager authenticationManager(AuthenticationConfiguration a) throws Exception { //AuthenticationManager
