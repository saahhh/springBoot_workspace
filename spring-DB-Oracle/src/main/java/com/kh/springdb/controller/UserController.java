...
public class UserController {
	...
	*/
	...
	//GetMapping() 추가해줘야 업데이트 가능
		//사용자 정보를 가져와서 모델에 추가해줘야함
	//return "user-update";
	@GetMapping("/user-update/{id}")
	public String updateUserForm(@PathVariable int id, Model model) {
		//사용자 정보를 가져와서 모델에 추가해줘야 하기 때문에 사용자 정보를 가지고 오는 모델 추가 코드를 작성
		User user = userService.getUserById(id);
		model.addAttribute("user", user);
		return "user-update";
	}
	
	//id에 해당하는 update문 사용해서 수정하기
	//api : 
	...
	public String updateUser(@PathVariable int id, @ModelAttribute("user") @Validated User user, BindingResult result) {
		...
		*/
		user.setMno(id); //경로에서 받은 id를 이용해서 사용자 정보를 업데이트한다
		userService.updateUser(user);
		return "redirect:/user-info";
	}
	
	/*GET과 POST를 한 번에 처리할 수 있는 방법이 없을까?
	  => @RequestMapping 을 사용해서 method GET과 POST를 한 번에 묶어서 해결하자
	...
	/*
	//@DeleteMapping 에는 @RequestMapping이 포함되어 있는 어노테이션
	@DeleteMapping("/user-delete/{id}")
	public String deleteUser(@PathVariable int id, @ModelAttribute("user") @Validated User user, BindingResult result) {
		userService.deleteUser(id);
		return "redirect:/user-info";
	}
	*/
...
@ModelAttribute("값")
	Thymeleaf 뷰에서 설정한 값의 이름을 사용해서 모델 속성에 엑서스 할 수 있음
...
@RequestMapping : 사용자의 HTTP요청을 특정 메서드와 할 수 있도록 매핑(감싸주는) 역할을 한다

	value = ""	value는 사용자가 요청할 때 주고받는 url을 작성해준다
		예를들어, value="/user-delete/{id}" 값이 있을 때 {id}는 PathVariable(패스(경로)변수)로 실제 요청할 경우 해당 url위치로 들어오는 값을 변수로 활용할 수 있다
	method = "" 메서드에서 처리할 HTTP요청을 메서드에 지정
		예를들어, RequestMethod.GET
				RequestMethod.POST 사용할 수 있다
				
				RequestMethod.GET
					HTTP GET 메서드는 주로 DB에 정보를 요청하기위해 사용된다
					사이트에서 URL을 통한 직접 요청이나 링크를 클릭해야하는 상황에서 GET메서드가 사용된다.
					데이터를 서버로 전송하지는 않고, 주로 데이터를 요청하거나 조회할 때만 사용할 수 있다.
				RequestMethod.POST
					주로 서버로 데이터를 제출하기 위해 사용된다
					데이터를 HTML 본문에 담아 전송하기 때문에 GET보다는 대량의 데이터를 처리하기에 적합하다
